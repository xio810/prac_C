...
int group_data_check(struct Group_data data)
{
    int alphabet[26] = {0};

    for (int j = 0; j < data.length; j++) // 입력받은 문자열 반복하는 반복문
    {
        // aabb 입력됐다고 가정
        if (j > 0 && data.str[j] != data.str[j - 1])
        {
            if (alphabet[data.str[j] - 'a'] == 1)
            {
                return 0;
            }
        }
        alphabet[data.str[j] - 'a'] = 1;
    }
    return 1;
}
...
int main()
{
    ...
    {
        struct Group_data data_m;
        scanf("%s", data_m.str);
        data_m.length = strlen(data_m.str);
        ...
        int result = group_data_check(data_m);
